...
.步骤
. <<导航到网格管理器>>
. link:bare-metal-storagegrid-license.html["指定 StorageGRID 许可证信息"]
. link:bare-metal-add-sites-storagegrid.html["将站点添加到StorageGRID"]
. link:bare-metal-grid-network-subnets-storagegrid.html["指定网格网络子网"]
. link:bare-metal-approve-grid-nodes-storagegrid.html["批准待定网格节点"]
. link:bare-metal-ntp-server-storagegrid.html["指定NTP服务器信息"]
. link:bare-metal-dns-server-storagegrid.html["指定域名系统服务器信息"]
. link:bare-metal-system-passwords-storagegrid.html["指定 StorageGRID 系统密码"]
. link:bare-metal-review-config-complete-storagegrid-install.html["查看您的配置并完成安装"]
