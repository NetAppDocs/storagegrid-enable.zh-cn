...
NOTE: 表中的数字适用于每个 StorageGRID 服务类型，而不适用于整个网格或每个物理主机。根据部署选择，我们将在本文档的后面部分link:bare-metal-prerequisites-install-storagegrid.html#physical-host-layout-and-requirements["物理主机布局和要求"]计算每个物理主机的数量。++ ++ 标有星号的路径或文件系统将由安装程序在 StorageGRID 容器中创建。管理员不需要手动配置或创建文件系统，但主机需要块设备来满足这些要求。换句话说，块设备应通过使用命令 `lsblk`显示，但不应在主机操作系统内进行格式化或挂载。++ ++
...
|===
在此示例中，每个容器类型都需要下表中显示的磁盘大小。每个物理主机的要求在本文档后面的 link:bare-metal-prerequisites-install-storagegrid.html#physical-host-layout-and-requirements["物理主机布局和要求"] 中进行了说明。
...
如果您不使用 iSCSI 存储，则必须确保 host1、host2 和 host3 包含足够大小的块设备以满足其要求。有关 host1、host2 和 host3 的存储要求，请参阅 link:bare-metal-prerequisites-install-storagegrid.html#disk-sizes-per-container-type["每个容器类型的磁盘大小"]。
...
----
. 使用步骤 2 中的启动器名称，将存储设备上的 LUN（数量和大小如 link:bare-metal-prerequisites-install-storagegrid.html#storage-requirements["存储要求"] 表中所示）映射到每个存储节点。
