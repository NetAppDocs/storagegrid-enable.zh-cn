...
在开始部署之前，让我们检查一下 NetApp StorageGRID 软件的计算、存储和网络要求。StorageGRID 作为容器化服务在 Podman 或 Docker 中运行。在此模型中，某些要求是指主机操作系统（托管 Docker 的操作系统，用于运行 StorageGRID 软件）。某些资源直接分配给每个主机内运行的 Docker 容器。在此部署中，为了最大限度地提高硬件使用率，我们为每个物理主机部署了两项服务。有关详细信息，请继续阅读下一节 link:bare-metal-prerequisites-install-storagegrid.html["安装StorageGRID的前提条件"]。
