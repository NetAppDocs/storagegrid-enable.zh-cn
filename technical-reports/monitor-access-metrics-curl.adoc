...
----
+
现在，您可以使用授权令牌字符串通过 cURL 访问指标。访问指标的流程类似于 link:monitoradvanced-monitor-storagegrid.html#export-metrics-through-the-api["StorageGRID中的高级监控"] 节中的步骤。但是，出于演示目的，我们展示了在"指标"类别中选择 GET /grid/metric-labels/{label}/values 的示例。
