...
请务必阅读产品文档 https://www.veeam.com/documentation-guides-datasheets.html?productId=8&version=product%3A8%2F221["Veeam备份和复制"^]和 https://docs.netapp.com/us-en/storagegrid/["StorageGRID"^]在你开始之前。 Veeam 提供了计算器，用于了解 Veeam 基础架构的大小和容量要求，这些计算器应在确定StorageGRID解决方案的大小之前使用。请始终在 Veeam Ready Program 网站上查看 Veeam- NetApp验证的配置，以了解 https://www.veeam.com/alliance-partner-technical-programs.html?alliancePartner=netapp1&page=1["Veeam Ready对象、对象不可变性和存储库"^]。
...
对于 Veeam 部署，建议使用带有最新修补程序的NetApp StorageGRID 11.9 或 12.0 版本。始终建议保持最新状态并为StorageGRID系统应用最新的修补程序。
...
StorageGRID是一个安全的多租户存储系统。建议为 Veeam 工作负载创建专用租户。可以选择分配存储配额。作为最佳实践，启用“使用自己的身份源”。使用适当的密码保护租户根管理用户。  Veeam Backup 12 要求 S3 存储桶具有强一致性。 StorageGRID提供在存储桶级别配置的多个一致性选项。对于 Veeam 从多个位置访问数据的多站点部署，请选择“strong-global”。如果 Veeam 备份和恢复仅在单个站点进行，则一致性级别应设置为“strong-site”。有关存储桶一致性级别的更多信息，请查看 https://docs.netapp.com/us-en/storagegrid/s3/consistency-controls.html["文档。"]。要使用StorageGRID进行 Veeam 不变性备份，必须在全局启用 S3 对象锁，并在存储桶创建期间在存储桶上进行配置。
...
Veeam 提供删除请求率调整和备份删除过程的调度。为了进一步调整删除性能，您可以禁用同步删除并让 ILM 扫描器管理对象的最终删除。

禁用同步删除的步骤

. 打开StorageGRID网格管理器。
. 在右上角，选择问号，然后选择 API 文档。
. 在右上角，单击私有 API 文档页面链接。
. 扩展 ilm-advanced。
. 选择 GET ilm-advanced。
. 选择“试用”，然后“执行”。
. 查看响应结果。
+
.. 如果值为空，则表示正在使用默认的 ilm-advanced 值。
.. 如果值不为空，则表示自定义 ILM 高级值正在使用中。复制“数据”之后的所有输出：，从 { 开始直到倒数第二个 }。
+
... 将其保存在某些文本编辑器中。
+
响应示例：
...
+
image:veeam-bp/get_ilm_adv_before.png["获取 ILM-Advanced 输出"]
...
. 选择“试用”开始编辑 API 主体。
+
.. 默认情况下，API 主体将包含默认值，而不包含任何先前配置的自定义值。这就是执行步骤 5-7 非常重要的原因。


. 如果在步骤 5-7 中发现非默认值，则用步骤 7 中保存的输出替换 API 主体。。否则，如果步骤 5-7 中的值为空，则保留 API 主体不变。
. 在API主体框中调整以下参数：
+
.. 将同步值设置为false。
+
...
+
image:veeam-bp/put_ilm_adv.png["PUT ILM-高级输入"]
...
* link:https://docs.netapp.com/us-en/storagegrid/["NetApp StorageGRID产品文档"^]
