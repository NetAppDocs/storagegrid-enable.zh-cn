...
image::obj_lock_en.png[启用网格范围对象锁定]
...
image::veeam_concur_limit.png[Veeam并发任务限制]

按照Veeam控制台中有关备份作业配置的Veeam文档启动向导。添加VM后、选择SOBR存储库。

image::veeam_1.png[备份作业]

单击高级设置并将存储优化设置更改为4 MB或更大。应启用数据压缩和重复数据删除。根据需要更改子系统设置并配置备份作业计划。

image::veeam_blk_sz.png[自动生成的计算机问题描述的屏幕截图,320,375]
